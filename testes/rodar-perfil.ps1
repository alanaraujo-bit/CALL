# Conduz a gestao de perfil na aplicacao real, dentro do motor Chromium.
# Precisa do servidor de sinalizacao: metade do que se prova aqui — o cartao de
# alguem, a troca de perfil chegando de fora — so existe entre duas pessoas.
...
$perfil = Join-Path $env:TEMP ("call-perfil-" + (Get-Random))
...
$navegador = Start-Process $edge -PassThru -ArgumentList @(
  ...
  "http://127.0.0.1:8123/testes/perfil.html"
)
...
Write-Output ""
if ($r.falhas -eq 0) { Write-Output "Perfil: todos os testes passaram."; exit 0 }
Write-Output ("Perfil: {0} teste(s) falharam." -f $r.falhas)
exit 1
